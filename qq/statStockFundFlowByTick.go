...
func SaveTickAsExcel(stockCode string) {
	...
	for n := 0; ; n++ {
		...
		for m := 0; m < len(tick); m++ {
			...
			v := util.ToInt(list[4]) * 100
			a := util.ToInt(list[5])
			f := util.ToFloat(list[3])
			if list[6] == "B" || f > 0 {
				if v >= 500000 || a >= 1000000 {
					buy_cdd = buy_cdd + a
				} else if v >= 100000 || a >= 200000 {
					buy_dd = buy_dd + a
				} else if v >= 20000 || a >= 50000 {
					buy_zd = buy_zd + a
				} else {
					buy_xd = buy_xd + a
				}
			} else if list[6] == "S" || f < 0 {
				if v >= 500000 || a >= 1000000 {
					sell_cdd = sell_cdd + a
				} else if v >= 100000 || a >= 200000 {
					sell_dd = sell_dd + a
	...
	cell = row.AddCell()
	cell.SetValue((buy_cdd - sell_cdd) / 10000.0)
	cell = row.AddCell()
	cell.SetValue(buy_dd / 10000.0)
	cell = row.AddCell()
	cell.SetValue(sell_dd / 10000.0)
	cell = row.AddCell()
	cell.SetValue((buy_dd - sell_dd) / 10000.0)
	...
	cell = row.AddCell()
	cell.SetValue((buy_zd - sell_zd) / 10000.0)
	cell = row.AddCell()
	cell.SetValue(buy_xd / 10000.0)
	cell = row.AddCell()
	cell.SetValue(sell_xd / 10000.0)
	cell = row.AddCell()
	cell.SetValue((buy_xd - sell_xd) / 10000.0)
	cell = row.AddCell()
	cell.SetValue(other / 10000.0)
