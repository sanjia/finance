...
func StatStockFundFlowByTick() {
	stockCode := "600666" //os.Args[1];
	var buyAccount int64
	var sellAccount int64
	var otherAccount int64
	buyAccount = 0
	sellAccount = 0
	otherAccount = 0
	...
	for n := 1; ; n++ {
		...
		for m := 0; m < len(tick); m++ {
			...
			if list[6] == "B" {
				buyAccount = buyAccount + util.ToInt(list[5])
			} else if list[6] == "S" {
				sellAccount = sellAccount + util.ToInt(list[5])
			} else {
				otherAccount = otherAccount + util.ToInt(list[5])
			}
		}
		fmt.Printf("sell account:%d,buy account:%d,other account:%d\r\n", sellAccount, buyAccount, otherAccount)
	}
}
